...
export function RegistruClient() {
  ...
  const exportToExcelCompatibil = () => {
    ...
    const rows = documente.map(d => [
      ...
      `"${d.continut?.replace(/"/g, '""')}"`, d.compartiment, d.destinatar, d.nr_conex, d.indicativ_dosar
    ]);

    // Folosim separator ";" pentru Excel RO și adăugăm BOM pentru diacritice
    const csvContent = "\uFEFF" + headers.join(";") + "\n" + rows.map(e => e.join(";")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", `Registru_Teius_2026_${new Date().toLocaleDateString()}.csv`);
      ...
      <div className="max-w-[1700px] mx-auto">
        <header className="bg-white p-6 rounded-[2.5rem] shadow-sm flex justify-between items-center mb-8 px-10 border border-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black uppercase tracking-tighter leading-none">REGISTRATURA <span className="text-blue-600">LICEULUI TEIUȘ</span></h1>
              <p className="text-slate-400 font-bold text-[9px] uppercase tracking-widest mt-1">Sistem Gestiune • 2026</p>
            </div>
          </div>
          <div className="flex gap-4">
             <button onClick={exportToExcelCompatibil} className="bg-emerald-500 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase flex items-center gap-2 hover:bg-emerald-600 transition-all"><FileSpreadsheet size={16}/> Salvare Excel</button>
            ...
            <table className="w-full text-left table-fixed min-w-[1300px]">
              <thead className="text-[9px] uppercase text-slate-400 font-black bg-slate-50/50 border-b">
                <tr>
                  ...
                  <th className="px-8 py-4 w-32 text-center text-blue-600">Conex / Ind.</th>
                  <th className="px-8 py-4 w-28 text-right">Acțiuni</th>
                </tr>
              </thead>
              <tbody className="text-[10px] divide-y divide-slate-100 uppercase font-bold">
                {documente.filter(d => d.numar_inregistrare?.toString() === search || d.emitent?.toLowerCase().includes(search.toLowerCase()) || d.continut?.toLowerCase().includes(search.toLowerCase())).map((doc) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                  ...
                  <div className="space-y-4">
                    <label className="block text-[10px] font-black uppercase text-slate-400">Compartiment
                      <div className="flex gap-2 mb-2 mt-2">
                        {["SECRETARIAT", "CONTABILITATE", "APP", "ALTELE"].map(v => (
                          <button key={v} onClick={() => setForm({...form, compartment: v})} className={`text-[8px] px-3 py-1 rounded-lg font-black transition-all ${form.compartiment === v ? 'bg-orange-500 text-white shadow-md' : 'bg-orange-50 text-orange-600 hover:bg-orange-100'}`}>{v}</button>
                        ))}
                      </div>
                      <input type="text" placeholder="Specificați compartimentul..." value={form.compartiment} onChange={(e) => setForm({...form, compartment: e.target.value.toUpperCase()})} className="w-full bg-slate-50 border-none p-4 rounded-2xl font-bold text-xs outline-none focus:ring-2 ring-orange-500" />
                    </label>
                    ...
                    <div className="bg-blue-50/50 p-4 rounded-[2.5rem] border border-blue-100">
                      <span className="text-[9px] font-black uppercase text-blue-600 mb-2 block leading-tight">Nr. de înregistrare la care se conexează și indicativul dosarului</span>
