      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-6 text-slate-900">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-4xl shadow-2xl relative border-[12px] border-slate-50">
            ...
            {activeTab === 'general' ? (
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block tracking-widest">Data</label>
                ...
                <div>
                  ...
                  <input type="text" placeholder="EX: TEIUȘ - ALBA IULIA..." value={form.destinatar} onChange={e => setForm({...form, destinatar: e.target.value})} className="w-full p-6 bg-slate-50 rounded-[2rem] font-black border-2 border-slate-100 uppercase outline-none" />
                </div>
              </div>
            ) : (
              <div className="space-y-6">
            ...
            <button onClick={handleSave} disabled={loading} className="w-full bg-blue-600 text-white p-6 rounded-[2rem] font-black text-lg uppercase shadow-xl mt-10 transition-all">{loading ? 'SALVARE...' : 'Salvează în Registru'}</button>
          </div>
        </div>
      )}
