        ...
        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md text-center border-t-8 border-blue-600">
          <div className="flex justify-center mb-6">
            <div className="w-20 h-20 bg-blue-50 rounded-3xl flex items-center justify-center border-2 border-blue-100 shadow-inner">
               <img src="https://cdn-icons-png.flaticon.com/512/2641/2641409.png" alt="Logo" className="w-14 h-14 object-contain" />
            </div>
          </div>
      ...
      <div className="max-w-[98%] mx-auto flex-grow">
        <header className="flex justify-between items-center mb-10 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          <div className="flex items-center gap-6">
             <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 shadow-sm overflow-hidden text-blue-600 font-black italic text-xl">LT</div>
             <div>
        ...
        <div className="mb-10 flex gap-6">
            <button onClick={() => { setEditingId(null); setForm({ data: new Date().toISOString().split('T')[0], data_sfarsit: '', nr_manual: '', emitent: '', continut: '', destinatar: '', data_expediere: '', conex: '', indicativ_dosar: '', compartiment: currentUser, observatii: '' }); setShowForm(true); }} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 text-left hover:shadow-xl transition-all w-full md:w-1/3">
              ...
              <h3 className="font-black text-2xl text-slate-800 mb-1">
                {activeTab === 'general' ? 'Adaugă Înregistrare' : activeTab === 'decizii' ? 'Adaugă Decizie' : activeTab === 'delegatii' ? 'Adaugă Delegație' : 'Adaugă Registru'}
              </h3>
            </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50/50 text-[9px] font-black uppercase text-slate-400">
                {activeTab === 'general' ? (
                  ...
                  <tr>
                    <th className="px-4 py-5">Nr. Document</th><th className="px-4 py-5">Data</th>
                    <th className="px-4 py-5">Conținut</th><th className="px-4 py-5">Observații</th>
                    <th className="px-4 py-5 text-right">Editare</th>
                  </tr>
                )}
              </thead>
              <tbody className="divide-y divide-slate-50 text-[11px] font-bold text-slate-600">
                {data.filter(i => (i.continut || i.emitent || '').toLowerCase().includes(search.toLowerCase())).map(item => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    {activeTab === 'general' ? (
                      <>
                        <td className="px-4 py-4"><span className={`px-3 py-1.5 rounded-xl text-[9px] font-black text-white ${item.tip_document === 'INTRARE' ? 'bg-emerald-500' : 'bg-blue-600'}`}>{item.tip_document}</span></td>
                    ...
                    <td className="px-4 py-4 text-right">
                      <button onClick={() => { setEditingId(item.id); setForm({...item, data: item.data_emitere || item.creat_la || item.data_inceput}); setShowForm(true); }} className="text-slate-300 hover:text-blue-600"><Edit2 size={14}/></button>
                    </td>
                    {activeTab === 'delegatii' && <td className="px-4 py-4 text-[10px] text-slate-400 italic">{item.creat_de}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-4xl shadow-2xl relative border-[12px] border-slate-50">
            ...
            <div className="grid grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div>
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">{activeTab === 'delegatii' ? 'Titular Delegație' : 'Emitent'}</label>
              ...
              <div className="space-y-6">
                <div>
                   <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">{activeTab === 'delegatii' ? 'Ruta / Scop' : 'Destinatar'}</label>
                   <input type="text" placeholder="SCRIE..." value={form.destinatar} onChange={e => setForm({...form, destinatar: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-black border-2 border-slate-100 uppercase outline-none" />
                </div>
                {activeTab !== 'delegatii' && (
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Conținut</label>
                    <textarea value={form.continut} onChange={e => setForm({...form, continut: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-bold h-32 uppercase outline-none border-2 border-slate-100" />
                  </div>
                )}
              </div>
            </div>
            
            <button onClick={handleSave} disabled={loading} className="w-full bg-blue-600 text-white p-6 rounded-[2rem] font-black text-lg uppercase shadow-xl mt-10 transition-all">{loading ? 'SALVARE...' : 'Finalizare Înregistrare'}</button>
          </div>
        </div>
      )}
      ...
      {allocatedNumber && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl flex items-center justify-center z-[100] p-6">
           <div className="bg-white rounded-[3rem] p-12 w-full max-w-lg text-center border-[12px] border-emerald-50 shadow-2xl">
              ...
              <h2 className="text-xl font-black text-slate-800 uppercase mb-4 tracking-tighter italic">Înregistrare Reușită!</h2>
