...
const formatDate = (dateString) => {
  ...
  const [form, setForm] = useState({
    ...
    nume_prenume: '', ruta: '' // Câmpuri noi pentru Delegații
  });
 ...
 const fetchData = useCallback(async () => {
    ...
    if (activeTab === 'delegatii') tableName = 'registru_delegatii'; // Tabelul nou
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-6 text-slate-800 flex flex-col">
      <div className="max-w-[98%] mx-auto flex-grow">
        ...
        {activeTab !== 'general' && (
          <div className="mb-10">
            <button onClick={() => { setEditingId(null); setForm({ data: new Date().toISOString().split('T')[0], data_sfarsit: '', nr_manual: '', emitent: '', continut: '', destinatar: '', data_expediere: '', conex: '', indicativ_dosar: '', compartiment: currentUser, observatii: '', nume_prenume: '', ruta: '' }); setShowForm(true); }} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 text-left hover:shadow-xl transition-all w-full md:w-1/3">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden mb-12 text-slate-900">
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    {activeTab === 'general' ? (
                      <>
                        ...
                        <td className="px-4 py-4 uppercase break-words min-w-[200px]">{item.continut}</td>
                      ...
                      <>
                        <td className="px-4 py-4 text-blue-600 font-black">{item.numar_inregistrare}</td>
                        <td className="px-4 py-4 uppercase font-black">{item.nume_prenume}</td>
                      ...
                      <>
                        <td className="px-4 py-4"><span className="bg-indigo-600 px-3 py-1.5 rounded-xl text-[9px] font-black text-white">{item.tip_document}</span></td>
                        <td className="px-4 py-4 text-blue-600 font-black">{item.numar_inregistrare}</td>
                    ...
                    <td className="px-4 py-4 text-right">
                      <button onClick={() => { 
                        ...
                        setForm({...item, data: item.data || item.data_emitere || item.creat_la || item.data_inceput});
                        setShowForm(true); 
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    ...
                    <input type="text" placeholder="NUME COMPLET..." value={form.nume_prenume} onChange={e => setForm({...form, nume_prenume: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-black border-2 border-slate-100 uppercase outline-none" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input type="text" placeholder="EX: TEIUȘ - ALBA IULIA - TEIUȘ..." value={form.ruta} onChange={e => setForm({...form, ruta: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-black border-2 border-slate-100 uppercase outline-none" />
                </div>
              </div>
            ) : activeTab === 'general' ? (
               /* Restul codului de formular rămâne neschimbat */
               <div className="text-xs italic text-slate-400">... restul câmpurilor existente ...</div>
            ) : null}
            
            <button onClick={handleSave} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white p-6 rounded-[2rem] font-black text-lg uppercase shadow-xl mt-10 transition-all">{loading ? 'SALVARE...' : 'Salvează'}</button>
          </div>
        </div>
      )}
