...
const formatDate = (dateString) => {
  ...
  const handleLogin = () => {
    if (!currentUser) {
      alert("Vă rugăm să selectați compartimentul!");
      return;
    }
    if (pass === 'liceulteius2026') {
      setIsAuth(true);
    } else {
  ...
  const exportToExcel = () => {
    ...
    if (activeTab === 'general') {
      headers = "Tip;Nr Inregistrare;Data;Emitent;Continut;Compartiment;Destinatar;Data Expediere;Conex/Ind";
      ...
      headers = "Tip;Nr Document;Data;Continut;Observatii;Creat De";
      rows = data.map(i => `"${i.tip_document}";"${i.numar_inregistrare}";"${i.data_emitere}";"${i.continut}";"${i.observatii}";"${i.creat_de}"`);
    } else if (activeTab === 'delegatii') {
      headers = "Nr Delegatie;Nume si Prenume;Ruta;Data;Creat De";
      rows = data.map(i => `"${i.numar_inregistrare}";"${i.emitent}";"${i.destinatar}";"${i.data_emitere}";"${i.creat_de}"`);
    } else if (activeTab === 'registre') {
      headers = "Nr Registru;Data Inceput;Continut;Data Terminare;Observatii;Creat De";
      rows = data.map(i => `"${i.numar_inregistrare}";"${i.data_inceput}";"${i.continut}";"${i.data_sfarsit}";"${i.observatii}";"${i.creat_de}"`);
    }
  ...
  const handleSave = async () => {
    ...
    const tableName = 
      ...
      activeTab === 'delegatii' ? 'registru_delegatii' :
    ...
    if (activeTab === 'general') {
      ...
      payload = {
        ...
        emitent: form.emitent.toUpperCase(), // Nume si Prenume
        destinatar: form.destinatar.toUpperCase(), // Ruta
        creat_de: currentUser,
        anul: new Date().getFullYear(),
        compartiment: currentUser
      };
    ...
    return (
      <div className="min-h-screen bg-[#f1f5f9] flex items-center justify-center p-6 text-slate-900">
        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md text-center border-t-8 border-blue-600">
          ...
          <select 
            className="w-full p-4 bg-slate-50 rounded-2xl mb-4 font-bold border-2 border-slate-100 outline-none"
            value={currentUser}
          ...
          <input 
            type="password" 
            placeholder="Parolă" 
            className="w-full p-4 bg-slate-50 rounded-2xl mb-6 text-center font-bold border-2 border-slate-100 outline-none"
            value={pass}
          ...
          <button 
            onClick={handleLogin}
            className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all"
          >
            Intră în sistem
          </button>
        ...
        <header className="flex justify-between items-center mb-10 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="bg-emerald-600 text-white px-5 py-3 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all"
            >
            ...
            <button 
              onClick={() => {
                ...
                setForm({
                  ...
                  emitent: '', continut: '', destinatar: '', 
                  data_expediere: '', conex: '', indicativ_dosar: '', 
                  compartiment: currentUser, observatii: ''
                });
                setShowForm(true);
              }}
            ...
            {['INTRARE', 'IESIRE', 'REZERVAT'].map(t => (
              <button 
                ...
                onClick={() => {
                  setFormType(t);
                  setEditingId(null);
                  setForm({...form, compartiment: currentUser, data: new Date().toISOString().split('T')[0]});
                  setShowForm(true);
                }}
                className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 text-left hover:shadow-xl transition-all"
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white mb-4 ${t==='INTRARE'?'bg-emerald-500':t==='IESIRE'?'bg-blue-500':'bg-orange-500'}`}>
                  <Plus size={24} strokeWidth={3}/>
                </div>
          ...
          <div className="p-6 border-b border-slate-50 flex justify-between items-center text-slate-900">
             <div className="flex items-center gap-3 bg-slate-50 px-5 py-3 rounded-2xl w-96 border border-slate-100">
                ...
                <input 
                  ...
                  className="bg-transparent outline-none font-bold text-slate-600 w-full text-sm"
                  value={search}
              ...
              <thead className="bg-slate-50/50 text-[9px] font-black uppercase text-slate-400">
                {activeTab === 'general' ? (
                  <tr>
                    ...
                    <th className="px-4 py-5">Nr. Înregistrare</th>
              ...
              <tbody className="divide-y divide-slate-50 text-[11px] font-bold text-slate-600">
                {data.filter(i => (i.continut || i.emitent || '').toLowerCase().includes(search.toLowerCase())).map(item => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    {activeTab === 'general' ? (
                      <>
                        <td className="px-4 py-4">
                          <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black text-white ${item.tip_document === 'INTRARE' ? 'bg-emerald-500' : 'bg-blue-600'}`}>
                            {item.tip_document}
                          </span>
                        </td>
                      ...
                      <>
                        <td className="px-4 py-4 text-blue-600 font-black">{item.numar_inregistrare}</td>
                        <td className="px-4 py-4">{formatDate(item.data_emitere || item.data_inceput)}</td>
                        <td className="px-4 py-4 uppercase">{item.continut}</td>
                    ...
                    <td className="px-4 py-4 text-right">
                      <button 
                        onClick={() => {
                          ...
                          setForm({...item, data: item.data_emitere || item.creat_la || item.data_inceput});
                          setShowForm(true);
                        }}
                    ...
                    {activeTab === 'delegatii' && (
                      <td className="px-4 py-4 text-[10px] text-slate-400 italic">{item.creat_de}</td>
                    )}
                  </tr>
                ))}
              </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-6 text-slate-900">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-4xl shadow-2xl relative border-[12px] border-slate-50">
            ...
            <h2 className="text-3xl font-black text-slate-800 mb-6 uppercase tracking-tighter italic">
                {activeTab === 'general' ? 'Date Registru' : activeTab === 'delegatii' ? 'Date Delegație' : 'Date Document'}
            </h2>
            ...
            {activeTab === 'general' ? (
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="SCRIE..." value={form.emitent} onChange={e => setForm({...form, emitent: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-black border-2 border-slate-100 uppercase outline-none" />
                  </div>
                </div>
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Conținut</label>
                    <textarea value={form.continut} onChange={e => setForm({...form, ArrowDown: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-bold h-32 uppercase outline-none border-2 border-slate-100" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Compartiment</label>
                    <input type="text" placeholder="SCRIE..." value={form.compartiment || ''} onChange={e => setForm({...form, compartment: e.target.value})} className="w-full p-5 bg-slate-50 rounded-2xl font-black border-2 border-slate-100 uppercase outline-none" />
                  </div>
                </div>
              </div>
            ) : activeTab === 'delegatii' ? (
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block tracking-widest">Nume și Prenume Titular</label>
                    <input 
                      type="text" 
                      placeholder="NUME COMPLET..." 
                      value={form.emitent} 
                      onChange={e => setForm({...form, emitent: e.target.value})} 
                      className="w-full p-6 bg-slate-50 rounded-[2rem] font-black border-2 border-slate-100 uppercase outline-none" 
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full p-6 bg-slate-50 rounded-[2rem] font-black border-2 border-slate-100 outline-none" 
                    />
                  </div>
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl flex items-center justify-center z-[100] p-6 text-slate-900">
           <div className="bg-white rounded-[3rem] p-12 w-full max-w-lg text-center border-[12px] border-emerald-50 shadow-2xl">
              ...
              <div className="bg-slate-50 rounded-[2rem] p-6 mb-8 border-2 border-slate-100">
                <span className="text-6xl font-black text-blue-600">#{allocatedNumber}</span>
              </div>
              <button 
                onClick={() => setAllocatedNumber(null)} 
                className="w-full bg-slate-900 text-white p-5 rounded-2xl font-black uppercase tracking-widest"
              >
                Închide
              </button>
           </div>
