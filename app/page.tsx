    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans selection:bg-blue-100 selection:text-blue-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            {[
              ...
              <button key={t.id} onClick={() => { setTipDocument(t.id); setIsEditing(false); setNumarGenerat(null); setShowForm(true); }} 
                className="bg-white p-10 rounded-[3rem] shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all group border border-white relative overflow-hidden text-left">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-colors shadow-lg ${t.id === 'intrare' ? 'bg-emerald-500 text-white shadow-emerald-100' : t.id === 'iesire' ? 'bg-blue-500 text-white shadow-blue-100' : 'bg-amber-500 text-white shadow-amber-100'}`}>
                  {t.icon}
                </div>
                <h3 className="font-black text-2xl text-slate-800 mb-1">{t.label}</h3>
                <p className="text-slate-400 text-sm font-medium">Adaugă înregistrare</p>
              </button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-[4rem] shadow-2xl shadow-slate-200/50 border border-white overflow-hidden">
            <div className="p-10 bg-slate-50/50 border-b flex flex-col md:flex-row justify-between items-center gap-6">
               <button onClick={() => {
                  ...
                  link.click();
                }} className="bg-slate-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black tracking-widest hover:bg-blue-600 transition-all flex items-center gap-3 shadow-xl uppercase">
               ...
               <div className="relative w-full md:w-80">
                  ...
                  <input type="text" placeholder="Căutare..." className="w-full pl-14 pr-6 py-4 bg-white border-2 border-slate-100 rounded-2xl text-sm outline-none focus:border-blue-500 transition-all font-medium text-slate-900" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
               </div>
            </div>
                ...
                <tbody className="text-sm divide-y divide-slate-100">
                  {documente.filter(d => (d.emitent || "").toLowerCase().includes(searchTerm.toLowerCase()) || (d.numar_inregistrare?.toString() || "").includes(searchTerm)).map((doc) => (
                    <tr key={doc.id} className="hover:bg-slate-50 transition-all group">
                      ...
                      <td className="px-10 py-8 text-center flex justify-center gap-4 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => {
                        ...
                        <button onClick={async () => { if(confirm('Elimini?')) { await supabase.from('documente').delete().eq('id', doc.id); fetchDocumente(); } }} className="text-red-400 hover:scale-125 transition-transform"><Icons.Trash2 size={18} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xl flex items-center justify-center p-6 z-50 animate-in fade-in duration-300">
          <div className="bg-white rounded-[4rem] p-12 w-full max-w-xl shadow-2xl relative border border-white text-slate-900">
            {!numarGenerat ? (
              <div className="space-y-8">
                ...
                <h2 className="text-3xl font-black uppercase tracking-tight">Înregistrare</h2>
                <div className="flex gap-3 bg-slate-100 p-2 rounded-2xl">
                  {['intrare', 'iesire', 'rezervat'].map((t) => (
                    <button key={t} type="button" onClick={() => setTipDocument(t)} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${tipDocument === t ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{t}</button>
                  ))}
                </div>
                <div className="space-y-5">
                  <input type="date" value={formData.data} onChange={(e) => setFormData({...formData, data: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-50 p-5 rounded-2xl font-bold outline-none focus:border-blue-500 focus:bg-white transition-all text-slate-900" />
                  <input type="text" placeholder="Corespondent" value={formData.expeditor} onChange={(e) => setFormData({...formData, expeditor: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-50 p-5 rounded-2xl outline-none focus:border-blue-500 focus:bg-white transition-all font-bold text-slate-900" />
                  <textarea placeholder="Descriere" value={formData.continut} onChange={(e) => setFormData({...formData, continut: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-50 p-5 rounded-2xl outline-none focus:border-blue-500 focus:bg-white transition-all font-bold text-slate-900 min-h-[120px]" />
                </div>
                <button onClick={handleSave} disabled={loading} className="w-full bg-blue-600 text-white font-black py-6 rounded-[2rem] uppercase tracking-widest text-xs shadow-2xl hover:
