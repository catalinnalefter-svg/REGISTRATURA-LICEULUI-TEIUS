        ...
        <div className="bg-white rounded-[4rem] shadow-xl border border-white overflow-hidden">
          <div className="p-10 bg-slate-50/50 border-b flex flex-col md:flex-row justify-between items-center gap-6">
            <button onClick={() => {
              ...
              link.click();
            }} className="bg-slate-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black tracking-widest uppercase hover:bg-blue-600 transition-all shadow-lg">Export Document</button>
              ...
              <tbody className="text-sm divide-y divide-slate-100">
                {documente.filter(d => (d.emitent || "").toLowerCase().includes(searchTerm.toLowerCase()) || (d.numar_inregistrare?.toString() || "").includes(searchTerm)).map((doc) => (
                  <tr key={doc.id} className="hover:bg-slate-50/80 transition-all group">
                    <td className="px-10 py-8 font-black text-blue-600 text-lg">#{doc.numar_inregistrare}</td>
                    ...
                    <td className="px-10 py-8 text-center flex justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={async () => { if(confirm('Ștergi?')) { await supabase.from('documente').delete().eq('id', doc.id); fetchDocumente(); } }} className="text-red-400 hover:scale-125 transition-transform"><Icons.Trash2 size={18} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {!numarGenerat ? (
              <div className="space-y-8">
                ...
                <div className="space-y-5">
                  <input type="date" value={formData.data} onChange={(e) => setFormData({...formData, data: e.target.value})} className="w-full bg-
