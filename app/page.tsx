      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {['intrare', 'iesire', 'rezervat'].map((tip) => (
            <button key={tip} onClick={() => { setTipDocument(tip); setIsEditing(false); setShowForm(true); }} className={`bg-white p-8 rounded-[2.5rem] border-2 border-transparent shadow-sm text-left group transition-all hover:border-${tip === 'intrare' ? 'emerald' : tip === 'iesire' ? 'blue' : 'orange'}-500`}>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden">
          <div className="p-8 bg-slate-50/50 border-b flex flex-col md:flex-row justify-between items-center gap-6">
            <h2 className="font-black text-slate-700 flex items-center gap-2 uppercase text-xs tracking-[0.2em]"><Icons.List size={16} className="text-indigo-600" /> Jurnal Înregistrări</h2>
            <div className="flex items-center gap-3 w-full md:w-auto">
              <button onClick={exportToCSV} className="bg-emerald-600 text-white px-6 py-3 rounded-2xl text-xs font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 flex items-center gap-2"><Icons.Download size={14} /> EXPORT EXCEL</button>
              ...
              <tbody className="text-sm divide-y divide-slate-100">
                {documenteFiltrate.map((doc) => (
                  <tr key={doc.id} className="hover:bg-indigo-50/30 transition-colors group">
                    ...
                    <td className="px-8 py-5 text-center flex items-center justify-center gap-2 mt-2">
            ...
            {!numarGenerat ? (
              <div className="space-y-6">
                ...
                <div className="space-y-1">
                  <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tight">{isEditing ? 'Modifică' : 'Înregistrare Nouă'}</h2>
                  <p className="text-indigo-600 font-bold text-xs uppercase tracking-widest">Document: {tipDocument}</p>
                </div>
                <div className="space-y-4">
                  ...
                  <input type="text" placeholder="Emitent / Destinatar" value={formData.expeditor} onChange={(e) => setFormData({...formData, expeditor: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl focus:border-indigo-500 outline-none transition-all font-medium" />
                  <textarea placeholder="Conținut..." value={formData.continut} onChange={(e) => setFormData({...formData, continut: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl focus:border-indigo-500 outline-none transition-all font-medium" rows={3} />
                </div>
                <button onClick={handleSave} disabled={loading} className="w-full bg-indigo-600 text-white font-black py-5 rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 disabled:bg-slate-300 uppercase tracking-widest text-sm">
                  {loading ? 'Se procesează...' : isEditing ? 'Salvează Modificările' : 'Alocă Număr Înregistrare'}
                </button>
              </div>
            ) : (
              <div className="text-center py-10 animate-in zoom-in duration-500">
                <div className="w-24 h-24 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner"><Icons.CheckCircle size={56} /></div>
                <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Succes!</h2>
                <div className="text-8xl font-black text-indigo-600 mb-2 tracking-tighter">#{numarGenerat}</div>
